...
function App() {
  ...
  function startCallRecorder(stream) {
    ...
    const VAD_THRESHOLD = 30;  // raised from 45 — real speech hits 60+, noise stays below
    let peakVolume = 0;
    let speechFrames = 0;   // how many 100ms intervals had speech above threshold
    let totalFrames  = 0;   // total intervals in this window
    ...
    vadInterval = setInterval(() => {
      ...
      totalFrames++;
      if (avg > VAD_THRESHOLD) speechFrames++;
    ...
    function spawnRecorder() {
      ...
      peakVolume   = 0;
      speechFrames = 0;   // reset for this window
      totalFrames  = 0;
      ...
      rec.ondataavailable = async (e) => {
        ...
        if (peak < VAD_THRESHOLD) {
          pushLog(`🔕 Dropped — silence (peak ${peak.toFixed(1)}, threshold ${VAD_THRESHOLD})`);
          return;
        }
        // KEY FIX: drop trailing chunks where < 15% of the window had real speech.
        // This catches the 3rd chunk where user already stopped talking but
        // peak is still above threshold from the last 0.3s of audio bleed.
        if (speechRatio < 0.15) {
          pushLog(`🔕 Dropped — trailing silence (only ${(speechRatio * 100).toFixed(0)}% speech frames, peak ${peak.toFixed(1)})`);
          return;
        }
        ...
        pushLog(`📤 Sending ${(e.data.size / 1024).toFixed(1)}KB — peak ${peak.toFixed(1)} speech ${(speechRatio * 100).toFixed(0)}% (${(listenedMs / 1000).toFixed(1)}s)`);
