...
_TRIAGE_HINTS = tuple(
    t.strip().lower()
    for t in os.getenv(
        "TRIAGE_SYMPTOM_HINTS",
        "pain,ache,fever,cough,cold,headache,nausea,vomiting,dizzy,dizziness,rash,breath,breathing,weakness,fatigue,diarrhea,chest,stomach,throat",
    ).split(",")
    if t.strip()
)
_GREETING_ONLY_RE = re.compile(r"^\s*(hi|hello|hey|salam|assalamualaikum|aoa)\s*[!.?]*\s*$", re.IGNORECASE)
...
def _looks_like_symptom_text(text: str) -> bool:
    t = (text or "").strip().lower()
    if not t or _GREETING_ONLY_RE.match(t):
        return False
    return any(h in t for h in _TRIAGE_HINTS)
...
""")
    ...
    if not triage_active and not ctx.get("triage_completed"):
        last_human = _last_human_text(raw_messages)
        if _looks_like_symptom_text(last_human):
            triage_active = True
            extracted_symptom = extracted_symptom or last_human
            if not ctx.get("prime_complaint"):
                ctx["prime_complaint"] = extracted_symptom
                _advance_step(ctx)
                save_booking_context(session_id, ctx)
            print(f"🚦 [Supervisor] fallback triage trigger from symptom text: '{last_human[:80]}'")
            response = AIMessage(content="")
